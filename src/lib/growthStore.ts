import { mirror, SYNC_ENABLED } from './dbSync'

// Growth/account-planning data syncs through the single-table mirror (dbSync),
// not per-table Supabase writes — those never matched the schema. Forcing the
// direct client to null makes every function use the local path, which mirrors.
export const supabase = null as any
export const DB_ENABLED = SYNC_ENABLED

// ─── Local store (mirrored to Supabase on write, hydrated on boot) ──────────
...
export function localSet(key: string, data: any) {
  try {
    localStorage.setItem(`nudge_${key}`, JSON.stringify(data))
    mirror(`nudge_${key}`, data)   // write-through to Supabase (no-op if disabled)
  } catch {}
}
