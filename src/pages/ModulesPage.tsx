...
import { MODULES } from '../data/shared'
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <p className="eyebrow" style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--brand)', marginBottom: 6 }}>INTELLIGENCE PLATFORM</p>
        ...
        {MODULES.map(mod => {
          ...
          return (
            <div key={mod.id} className="card card-clickable" onClick={() => active && (mod as any).path && nav((mod as any).path)}
              ...
              <h3 style={{ fontSize: 16, color: 'var(--text-1)', margin: '0 0 8px' }}>{mod.title}</h3>
              ...
              {active ? (
                ...
                <span className="eyebrow" style={{ color: 'var(--text-3)', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.05em', fontSize: 12 }}>Coming Soon</span>
              )}
            </div>
          )
        })}
      </div>
      <div style={{ marginTop: 52, textAlign: 'center', fontSize: 11.5, color: 'var(--text-3)', letterSpacing: '0.06em' }}>© 2026 THE COMPANY INTELLIGENCE. ALL RIGHTS RESERVED.</div>
    </div>
  )
}
