...
import { ALL_EXECS } from '../../data/growthIndex'
...
const CATEGORIES = [
  ...
  'Company Selling Point','Action Plan','Relationship Score','Tasks',
...
function OneMinuteSummary({ exec, execId }: { exec:any; execId:string }) {
  const fields = [
    ...
    { key:'freyrSellingPoint', label:'Company Selling Point', icon:'', accent:'#EA580C' },
  ]
...
// ─── INDEGENE SELLING POINT ─────────────────────────────────────────────────
// Department field + content text
function CompanySellingPoint({ exec, execId }: { exec:any; execId:string }) {
  ...
  return (
    <div>
      <SectionHeader title="Company Selling Point" onSave={save} saving={saving}/>
      <div style={{ border:'1px solid var(--border)', borderRadius:8, background:'var(--bg-surface)' }}>
        <FieldRow label="Department">
          <FieldInput value={dept} onChange={setDept} placeholder="e.g. EMS"/>
        </FieldRow>
        <FieldRow label="Selling Point">
          <FieldInput value={content} onChange={setContent} placeholder="Enter Company selling point..." multiline/>
        </FieldRow>
      </div>
    </div>
  )
...
// ─── RELATIONSHIP SCORE ────────────────────────────────────────────────────
// Current + dropdown (Hot/Warm/Cold/Friends of Company) + Reason + history
function RelationshipScore({ exec, execId }: { exec:any; execId:string }) {
  ...
  const SCORES = [
    ...
    { label:'Friends of Company', color:'#10b981', bg:'rgba(16,185,129,0.1)' },
  ]
...
function SectionContent({ cat, exec, execId }: { cat:string; exec:any; execId:string }) {
  ...
  if (cat==='Company Selling Point')      return <CompanySellingPoint exec={exec} execId={execId}/>
...
<div class="page">
  ...
  <!-- Header -->
  <div class="header">
    ${avatarUrl(exec.name) ? `<img class="avatar" src="${avatarUrl(exec.name)}" alt="" style="object-fit:cover;" />` : `<div class="avatar">${esc(exec.name.split(' ').map((n:string)=>n[0]).join('').slice(0,2))}</div>`}
  ...
  ${section('One Minute Summary', [
    ...
    field('Company Selling Point', s.freyrSellingPoint || s.freyrSellingPoint || ''),
  ].join(''))}
  ...
  <!-- Company Selling Point -->
  ${exec.sellingPoint ? section('Company Selling Point', `
  ...
  <div class="footer">
    The Company Intelligence — Powered by Company Solutions · Confidential · ${new Date().toLocaleDateString('en-GB',{month:'long',year:'numeric'})}
  </div>
</div>
    ...
    <div>
      ...
      <div className="card" style={{ padding:'18px 24px', marginBottom:16 }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:16 }}>
          <Avatar name={exec.name} size={60} radius={30} bg={av} />
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:7, alignItems:'center' }}>
              ...
              <a href={/linkedin\.com/i.test(exec.linkedIn||'') ? exec.linkedIn : `https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(`${exec.name||''} ${exec.company||''}`.trim())}`}
                target="_blank" rel="noopener noreferrer" title="View LinkedIn Profile" style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:28, height:28, borderRadius:6, background:'#0A66C2', textDecoration:'none', flexShrink:0 }}>
