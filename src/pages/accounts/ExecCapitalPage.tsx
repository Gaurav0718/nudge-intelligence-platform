...
import { Bookmark, BookmarkCheck, Plus, Linkedin } from 'lucide-react'
import Avatar from '../../components/shared/Avatar'

/** LinkedIn first; if the stored link is not a LinkedIn URL, deep-link a LinkedIn people search. */
export function linkedInUrl(exec: any): string {
  const url = exec?.linkedIn || ''
  if (/linkedin\.com/i.test(url)) return url
  return `https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(`${exec?.name || ''} ${exec?.company || ''}`.trim())}`
}
import { ALL_EXECS, ACCOUNTS_LIST, ORG_BY_ACCOUNT } from '../../data/growthIndex'
import { getOrgChart, saveOrgChart } from '../../lib/growthStore'
import type { OrgPerson } from '../../lib/growthStore'
...
function ProperOrgCard({ person, isSelected, draggingId, editMode, onSelect, onEdit, onRemove, onDetach, x, y }: any) {
  ...
  return (
    <div ref={setRef}
      ...
      <Avatar name={person.name} size={44} radius={12} bg={av} style={{ marginRight:12 }} />
...
function OrgChart({ accountId }: { accountId: string }) {
  const defaultPeople = (ORG_BY_ACCOUNT[accountId] || []).map(p => ({
  ...
  function cycleCheck(ps: OrgPerson[], pid: string, newMgr: string): boolean {
    ...
    while (node !== null) {
      ...
      visited.add(node)
      const cur: string = node
      node = ps.find((p: any)=>p.id===cur)?.managerId ?? null
    }
    return false
  }
            ...
            {activePerson && (() => {
              ...
              return (
                <div style={{ width:CARD_W,background:'var(--bg-surface)',border:'2px solid var(--gold)',borderRadius:14,display:'flex',alignItems:'center',padding:'14px 16px',boxShadow:'0 20px 60px rgba(27,54,93,0.3)',transform:'rotate(1.5deg) scale(1.04)',cursor:'grabbing' }}>
                  <Avatar name={activePerson.name} size={44} radius={12} bg={av} style={{ marginRight:12 }} />
                  <div style={{ flex:1,minWidth:0 }}>
...
export default function ExecCapitalPage() {
  ...
  const [orgAccount, setOrgAccount] = useState('astrazeneca')
            ...
            return (
              <div key={exec.id} className="card" style={{ overflow:'hidden' }}>
                <div style={{ display:'flex',alignItems:'flex-start',gap:16,padding:'16px 20px',borderBottom:'0.5px solid var(--border)' }}>
                  <Avatar name={exec.name} size={52} radius={26} bg={av} />
                  <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ display:'flex',gap:8,flexShrink:0 }}>
                    <a href={linkedInUrl(exec)} target="_blank" rel="noopener noreferrer" title="LinkedIn profile"
                      style={{ display:'inline-flex',alignItems:'center',justifyContent:'center',width:30,height:30,borderRadius:7,background:'#0A66C2',color:'#fff',textDecoration:'none',flexShrink:0 }}>
                      <Linkedin size={14} />
                    </a>
                ...
                <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',padding:'14px 20px',gap:16 }}>
                  {[
                    ...
                    { label:'Company Mapping',    value:String(exec.freyrMapping),   numeric:true },
